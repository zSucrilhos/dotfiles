...
@click.option('-m', '--mode', default='fill', help="The mode to set the wallpaper. Available modes are:\n \n fill: Fill the image on the screen, preserving the aspect ratio.\ncenter: Center the image on the screen if it is too small.\nDefault: fill.")
            ...
            for file in wpp_list:
                ...
                if file_extension not in ('.jpg', '.png', '.tif', '.tiff'):
                    ...
                    if mode == "fill":
                        ...
                        os.system('feh --bg-center ' + directory + file + ' &> /dev/null') #  Apply the wallpaper
                        sleep(sleep_time) # Time the wallpaper can last
                    else:
                        os.system('feh --bg-fill ' + directory + file + ' &> /dev/null') #  Apply the wallpaper
                        sleep(sleep_time) # Time the wallpaper can last
